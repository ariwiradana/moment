...
const EventsComponent: FC<Props> = ({ state }) => {
  return (
        ...
        <div className="relative h-full w-full">
          {state.client?.gallery && state.client.gallery.length > 0 ? (
            <ImageShimmer
              ...
              src={
                state.client.gallery[state.client.gallery.length - 1] as string
              }
          ...
          <div className="grid md:grid-cols-2 gap-16 w-full">
            {state.client?.events.map((event) => (
              <div
                key={event.id}
                data-aos="zoom-in-up"
                className="border border-white p-8 lg:p-20 text-white rounded-[36px] lg:rounded-[100px]"
              >
                ...
                <div
                  className={`flex flex-col items-center justify-center gap-x-6 gap-y-4 mt-8 ${afacad.className}`}
                >
                  <Link href={event.address_url} target="_blank">
                    <Button title="Simpan Tanggal" icon={<BiSolidCalendar />} />
                  </Link>
                  <Link href={event.address_url} target="_blank">
                    <Button title="Petunjuk Lokasi" icon={<BiSolidMap />} />
                  </Link>
                </div>
              </div>
            ))}
